...
D2L.Util = {
	...
	getTokenUrl : function (host, path, port, parameters) {
		var parsedHost = url.parse(host),
			parsedPath = url.parse(path, true),
	 ...
	 * @param {String} _url Parameterized URL.
	 *
	 ...
	 * @this {D2L.Util}
	 */
	parseFromUrl : function (name, _url) {
		var param = (url.parse(_url, true).query || {})[name];
	...
	Sign : function (data, key) {
		...
		var base64 = hmac.digest('base64'),
			base64_u = this.base64Url(base64);
		return base64_u;
	},
...
D2L.ApplicationContext.prototype.createUrlForAuthentication =
	function (host, port, callback) {
		...
		var url = D2L.Util.getTokenUrl(host, D2L.Auth.AUTH_URL, port, params);
...
D2L.UserContext.prototype.createAuthenticatedUrl =
	function (path, method) {
		...
		var timestamp = this.skew + D2L.Util.getTimestamp();
		...
		var url = D2L.Util.getTokenUrl(this.host, path, this.port, params);
...
if (process.browser) {
	...
	Object.keys(D2L).forEach(function (prop) {
